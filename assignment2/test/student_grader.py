...
def check_dynamics(tracker, options):
    if options.test:
        from ekf_robot_answer import RobotEKF
    else:
        from ekf_robot import RobotEKF
    np.random.seed(16)
    setting = ['q1', 'sim', 'no_measure']
    ...
    kwargs={'robot_args': {
                ...
                'e_mode': setting[2], # 'no_measure', 'no_bearing', 'bearing'.
                's_R': np.c_[[0.02, 0.02, 0.005]],
                'e_R': np.diag([0.02, 0.02, 0.005]),
                },
            'init_args':{
                # 'land_cir_color': 'gray',
                },
    ...
    trajectory.append(env.robot.state) # Final state
...
@test('q2', points=30)
def check_ekf_prediction(tracker, options):
    if options.test:
        from ekf_robot_answer import RobotEKF
    else:
        from ekf_robot import RobotEKF
    np.random.seed(16)
    setting = ['q2', 'pre', 'no_measure']
...
@test('q3', points=30)
def check_ekf_prediction_no_bearing(tracker, options):
    if options.test:
        from ekf_robot_answer import RobotEKF
    else:
        from ekf_robot import RobotEKF
    np.random.seed(16)
    setting = ['q3', 'pre', 'no_bearing']
...
@test('q4', points=30)
def check_ekf_prediction_with_bearing(tracker, options):
    if options.test:
        from ekf_robot_answer import RobotEKF
    else:
        from ekf_robot import RobotEKF
    np.random.seed(16)
    setting = ['q4', 'pre', 'bearing']
